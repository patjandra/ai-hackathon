...
async function seed() {
  ...
  console.log(`Seeding ${patient.name}...`);
  ...
  // Clear existing
  const ids = await redis.zrange(`checkins:${patient.id}`, 0, -1);
  for (const id of ids) await redis.del(`checkin:${id}`);
  ...
  console.log(`\n✓ ${checkins.length} check-ins seeded`);
